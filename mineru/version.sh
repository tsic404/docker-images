#!/bin/bash
# mineru/version.sh
# Version detection for MinerU — OpenDataLab document parsing engine.
# Tag format: mineru-X.Y.Z-released
...
LAST_VERSION=mineru-3.1.11-released
OWNER="opendatalab"
REPO="MinerU"
DAYS_BEFORE=0

# mineru-specific tag filter
# mineru uses mineru-X.Y.Z-released format (stable releases only)
filter_mineru_tags() {
    local tags_json="$1"

    echo "$tags_json" | jq '
      map(select(
        .name | test("^mineru-[0-9]+\\.[0-9]+\\.[0-9]+-released$")
      ))
    '
}
...
main() {
    ...
    local filtered_tags
    filtered_tags=$(filter_mineru_tags "$all_tags")
