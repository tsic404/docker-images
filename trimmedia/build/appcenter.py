...
def resolve(app_name, version, platform):
...
def main():
    ...
    app, ver, dl = resolve(opts.app_name, opts.version, platform)
    ...
    bin_path = os.path.join(payload_dir, "trim-media")
    if not os.path.isfile(bin_path):
        raise SystemExit("解包结果里没有 trim-media：%s" % bin_path)
