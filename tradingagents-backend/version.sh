#!/bin/bash
# tradingagents-backend/version.sh
...
filter_tradingagents_backend_tags() {
    local tags_json="$1"
    echo "$tags_json" | jq '
      map(select(
...
main() {
    log_info "检测 $OWNER/$REPO (backend) 的新版本"
    ...
    local filtered_tags
    filtered_tags=$(filter_tradingagents_backend_tags "$all_tags")
